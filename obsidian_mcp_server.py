...
from obsidian_vectorstore import build_vector_store, delete_note, index_note
...
@app.post("/note", response_model=SearchResult)
@tool()
def upsert_note(payload: NotePayload) -> SearchResult:
    """Create or update a note and reindex it in the vector store."""

    filename = re.sub(r"[^\w\s-]", "", payload.title).strip().replace(" ", "-").lower() + ".md"
    file_path = Path(VAULT_PATH) / filename
    file_path.write_text(payload.content, encoding="utf-8")

    if payload.uuid:
        delete_note(store, payload.uuid)

    note_uuid = index_note(store, file_path)
    return SearchResult(uuid=note_uuid, source=str(file_path), content=payload.content)
